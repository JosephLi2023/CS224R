#!/usr/bin/env bash
# WebShop SOTA — Flat GRPO v1 — CLOUD-ORCHESTRATED variant.
#
# Difference vs. run_webshop_SOTA_flatGRPO_v1.sh
# ==============================================
# The sibling script runs a per-round bash for-loop locally that calls
# `modal run --detach infra/app_train_loop.py::train_loop_webshop`.
# If the laptop sleeps mid-run, only the in-flight round's Modal job
# completes; remaining rounds never launch.
#
# This script deploys both required apps once, then submits
# infra/app_orchestrator.py::orchestrate_rl_no_turnrd as a single
...
# orchestrator container's CPU+RAM. Per-round = train_loop only (no
# TurnRD trainer step — Flat GRPO doesn't need it).
#
# Resume support
# ==============
# Auto-resume via per-round sentinels under
# `${ADAPTER_DIR}/${RUN_PREFIX}_seed${SEED}_round{NN}_done.json`.
# Manual override: START_ROUND=<n> bash scripts/run_webshop_SOTA_flatGRPO_v1_cloud.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [[ -z "${SFT_ADAPTER}" ]]; then
  ...
  echo "           bash scripts/run_webshop_SOTA_flatGRPO_v1_cloud.sh"
  exit 1
fi

CONFIG=${CONFIG:-configs/flatGRPO_webshop_SOTA_10round_mlpr32_v1.json}
RUN_PREFIX=${RUN_PREFIX:-webshop_flatGRPO_v1_cloud}
ROUNDS=${ROUNDS:-10}
...
echo "═══════════════════════════════════════════════════════════════════════"
echo "WebShop SOTA — Flat GRPO v1 (CLOUD orchestrator)"
echo "  config             : ${CONFIG}"
