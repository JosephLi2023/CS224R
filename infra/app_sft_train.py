...
def sft_train(
    ...
    examples = load_sft_examples_from_directory(
        "/vol/data/webshop/human_trajs", min_reward=min_reward
    )
    summary = summarize_sft_dataset(examples)
    print(">>> Dataset summary:", summary)
    if not examples:
        raise RuntimeError("No SFT examples after filtering — adjust min_reward")
...
def main(
    ...
    res = sft_train.remote(
        ...
        log_every=log_every, run_name=run_name,
    )
