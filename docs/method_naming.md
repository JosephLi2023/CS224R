...
| **flatGRPO** | H-GRPO with α=0.0 (drops per-turn signal under proposal semantics where α weights A_turn) — the trivial-decomposer baseline. | `flat_grpo` (manifest), `configs/method_flat_grpo.json`, `flat_grpo_compare_seed11_*` (run names) |
