...
# ---------------------------------------------------------------------------
# Method D — Residual Decomposer dispatch
# ---------------------------------------------------------------------------


def test_loader_builds_residual_decomposer_with_init_gamma(tmp_path: Path) -> None:
    """`hgpo.decomposer == 'residual'` returns a ResidualDecomposer object
    with `gamma_prior` initialized from `cfg.residual.init_gamma`."""
    from src.algorithms.hgpo.decomposers.residual import ResidualDecomposer

    cfg = {
        "train": {"learning_rate": 1e-6},
        "hgpo": {"alpha": 0.5, "lambda_consistency": 0.1, "decomposer": "residual"},
        "turnrd": {
            "mode": 1,
            "layers": 2,
            "hidden_size": 32,
        ...
        "residual": {"init_gamma": 0.7},
    }
    trainer, refresh_fn, emit_path, embedder, judge_dec = build_trainer_from_config(
        cfg, policy=_StubPolicy()
    )
    assert isinstance(trainer, HGPOTrainer)
    assert isinstance(trainer.decomposer, ResidualDecomposer)
    # init_gamma honored.
    assert abs(trainer.decomposer.gamma_prior.detach().item() - 0.7) < 1e-6
    assert refresh_fn is None  # no ckpt_path was set
    assert emit_path == str(tmp_path / "replay.jsonl")
    assert embedder is not None and callable(embedder)
    assert judge_dec is None
    # gamma_prior shows up in parameters() so the trainer's AdamW updates it.
    assert any(p is trainer.decomposer.gamma_prior for p in trainer.decomposer.parameters())


def test_loader_residual_branch_requires_turnrd_block() -> None:
    """Residual reuses the turnrd config block; missing block raises."""
    cfg = {
        "train": {"learning_rate": 1e-6},
...
def test_loader_residual_default_init_gamma_when_block_missing(tmp_path: Path) -> None:
    """When `cfg.residual` is absent, init_gamma defaults to 1.0."""
    from src.algorithms.hgpo.decomposers.residual import ResidualDecomposer
    ...
    trainer, *_ = build_trainer_from_config(cfg, policy=_StubPolicy())
    assert isinstance(trainer.decomposer, ResidualDecomposer)
    assert abs(trainer.decomposer.gamma_prior.detach().item() - 1.0) < 1e-6
