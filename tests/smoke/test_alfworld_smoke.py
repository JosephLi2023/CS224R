...
class TestALFWorldSmoke(unittest.TestCase):
    def test_alfworld_reset_and_step_shape(self) -> None:
        env_cfg = {
            "name": "alfworld",
            "max_steps": 5,
            "task_split": "train",
        ...
        try:
            ...
            self.skipTest(f"ALFWorld dependency not available: {exc}")
            return
        ...
        # Use a valid action if available; otherwise fallback command string.
        action = state.valid_actions[0] if state.valid_actions else "look"
