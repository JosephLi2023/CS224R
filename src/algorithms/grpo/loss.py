...
Pure-Python reference for the trainer's inline torch math; checked against it
in tests/unit/test_grpo_loss.py.
"""
...
def clipped_ppo_term(
    ...
    """Schulman PPO clipped surrogate, per-token and negated to a loss:
...
def mask_mean(values: list[float], mask: list[float | int]) -> float:
    """Mean of `values` over positions where mask > 0; 0 (not NaN) when none."""
...
def kl_per_token(
    ...
    """Per-token KL k1 estimator new_logprob - old_logprob (unbiased, high-variance)."""
    if len(new_logprobs) != len(old_logprobs):
        raise ValueError(
            f"length mismatch: new={len(new_logprobs)} old={len(old_logprobs)}"
        )
...
def kl_k3_per_token(
    ...
    """Per-token KL k3 estimator (rho - 1) - log(rho), rho = exp(new - old)."""
    out: list[float] = []
